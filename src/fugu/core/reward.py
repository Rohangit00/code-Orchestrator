...
* **Cost efficiency** – penalises excessive token usage.
* **Latency** – gentle penalty for slow worker calls.
...
class RewardConfig:
    ...
    # Normalisation constants (used to scale raw values into ~[-1, 1])
...
class RewardCalculator:
    ...
    def compute(
        ...
        # 3. Token cost -------------------------------------------------
        normalized_cost = min(tokens_used / self.cfg.max_tokens_per_step, 1.0)
        reward -= self.cfg.w_cost * normalized_cost

        # 4. Latency ----------------------------------------------------
        normalized_latency = min(latency_ms / self.cfg.max_latency_ms, 1.0)
